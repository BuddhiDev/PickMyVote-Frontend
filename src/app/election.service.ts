...
export class ElectionService {
  ...
  public getOwnerOrgList(username:any, password:any, ownerId:Number):Observable<any>{
    ...
    return this._http.get<any>("https://pickmyvote.herokuapp.com/api/v1/getOrgsOfOwner/"+ ownerId,{headers});
  }

  getElectionList(username:any,password:any):Observable<Election[]>{
    const headers = new HttpHeaders({ Authorization: 'Basic ' + btoa(username + ':' + password) });
    return this._http.get<Election[]>("https://pickmyvote.herokuapp.com/getElections/",{headers})
  }

  createNewOrganization(username:string,password:string,organization:Organization):Observable<any>{
    const headers = new HttpHeaders({ Authorization: 'Basic ' + btoa(username + ':' + password) });
    return this._http.post<any>("https://pickmyvote.herokuapp.com/api/v1/createOrganization",organization,{headers})
  }

  createNewElection(username:string,password:string,election:Election):Observable<any>{
    const headers = new HttpHeaders({ Authorization: 'Basic ' + btoa(username + ':' + password) });
    return this._http.post<any>("https://pickmyvote.herokuapp.com/api/v1/createElection",election,{headers})
  }
  ...
  createNewPayment(username:string,password:string,payment:Payment):Observable<any>{
    const headers = new HttpHeaders({ Authorization: 'Basic ' + btoa(username + ':' + password) });
    return this._http.post<any>("https://pickmyvote.herokuapp.com/api/v1/createPayment",payment,{headers})
  }

  createNewCandidates(username:string,password:string,candidates:Candidate[]):Observable<any>{
    const headers = new HttpHeaders({ Authorization: 'Basic ' + btoa(username + ':' + password) });
    return this._http.post<any>("https://pickmyvote.herokuapp.com/api/v1/createCandidate",candidates,{headers})
  }

  createNewVoters(username:string,password:string,voters:Voter[]):Observable<any>{
    const headers = new HttpHeaders({ Authorization: 'Basic ' + btoa(username + ':' + password) });
    return this._http.post<any>("https://pickmyvote.herokuapp.com/createVoters",voters,{headers})
  }

  getElectionById(username:any,password:any,elecid:number):Observable<Election>{
    const headers = new HttpHeaders({ Authorization: 'Basic ' + btoa(username + ':' + password) });
    return this._http.get<Election>("https://pickmyvote.herokuapp.com/vote/"+elecid,{headers})
  }

  getcandidatesByEId(username:any,password:any,elecid:number):Observable<Candidate[]>{
    const headers = new HttpHeaders({ Authorization: 'Basic ' + btoa(username + ':' + password) });
    return this._http.get<Candidate[]>("https://pickmyvote.herokuapp.com/vote/candidates/"+elecid,{headers})
  }
